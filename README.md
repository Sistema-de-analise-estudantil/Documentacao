# Sistema de Análise Estudantil (SAE)

> **Repositório destinado a armazenar a documentação, planejamento e escopo do Trabalho de Conclusão de Curso (TCC) em Desenvolvimento de Sistemas.**

---

## Sobre o Projeto

O **SAE (Sistema de Análise Estudantil)** é um projeto desenvolvido como Trabalho de Conclusão de Curso (TCC). O objetivo principal do sistema é auxiliar alunos na criação de seus trabalhos acadêmicos e tornar esse conteúdo científico acessível.

---

## Estrutura do Repositório

Organizamos o repositório para facilitar a navegação entre a documentação de requisitos, modelagens e os relatórios oficiais. Abaixo está a explicação de cada diretório:

* **`Documentação Geral/`**: Contém o arquivo principal do TCC nos formatos  (`.docx`) e pronto para leitura (`.pdf`), estruturado conforme as normas da ABNT.
* **`Documentação de Requisitos/`**: Reúne os documentos de especificação de requisitos do sistema (versões antigas e atualizadas), detalhando o escopo funcional do software.
* **`Figuras/`**: Armazena diagramas e esquemas que auxiliam na compreensão do fluxo do sistema.
* **`Modelos/`**: Contém arquivos modelo de documentos usados ao longo do trabalho. Exemplo: (`Modelo - Fichamento.docx`).
* **`Resumo e Abstract/`**: Reúne os documentos de Resumo e Abstract usados na documentação Geral.

---

## Principais Documentos

Se você deseja ler a documentação, recomendamos os seguintes arquivos:

| Documento | Descrição | Formato |
| :--- | :--- | :---: |
| **Documentação Geral compilado em LaTeX** | `Documentação Geral/SAE-Documentação_do_TCC.pdf` | PDF |
| **Documentação Geral usando Microsoft Word** | `Documentação Geral/SAE - Documentação do TCC - 3DS M A.docx` | DOCX |
| **Especificação de Requisitos** | `Documentação de Requisitos/Documentação Requisitos V2.docx` | DOCX |
| **Diagrama de Casos de Uso** | `Figuras/Diagrama_de_Casos_de_Uso_TCC.pdf` | PDF |

---

## Tecnologias e Ferramentas Utilizadas (Planejadas)

* **Documentação:** Microsoft Word / Google Docs / LaTeX (Normas ABNT)
* **Controle de Versão e armazenamento:** Git & GitHub

---

## Autores

* **João Pedro** - *Documentação e Manutenção do repositório* - [@Joa6-Pedro](https://github.com/Joa6-Pedro)
* **Guilherme Pereira** - *Documentação e Manutenção do repositório* - [@SkyyPG](https://github.com/SkyyPG)

---
*Nota: Este repositório é focado estritamente na parte acadêmica, documental e de modelagem do projeto.*
